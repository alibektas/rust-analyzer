...
fn test_missing_module_code_action_in_json_project() {
    ...
    let tmp_dir = testdir::TestDir::new();
...
mod ratoml {
    ...
    use rust_analyzer::lsp::ext::CodeActionRequest;

    use serde_json::json;
    use tracing_subscriber::fmt::TestWriter;

    use crate::{
        support::{Project, Server},
        testdir::TestDir,
    };
...
```"#;
        ...
        fn new(
            ...
            let urls =
                fixtures.iter().map(|fixture| case.fixture_path(fixture)).collect::<Vec<_>>();
            case.urls = urls;
            case
        }
        ...
        fn fixture_path(&self, fixture: &str) -> Url {
            ...
            Url::parse(
                format!(
                    "file://{}",
                    path.to_str().unwrap().to_owned().replace("C:\\", "/c:/").replace('\\', "/")
                )
                .as_str(),
            )
            .unwrap()
        ...
        fn query(&self, query: QueryType, source_file_idx: usize) -> bool {
            match query {
                QueryType::AssistEmitMustUse => {
                    ...
                    let res =
                        serde_json::de::from_str::<CodeActionResponse>(res.to_string().as_str())
                            .unwrap();
...
assist.emitMustUse = true"#,
    ...
    #[test]
    fn inherit_config_from_ws_root() {
        let server = RatomlTest::new(
            vec![
                r#"
//- /p1/Cargo.toml
...
"#,
    ...
    #[test]
    fn add_immediate_child_to_ws_root() {
        let mut server = RatomlTest::new(
            vec![
                r#"
//- /p1/Cargo.toml
...
"#,
    ...
    #[test]
    fn rm_ws_root_ratoml_child_has_client_as_parent_now() {
        let mut server = RatomlTest::new(
            vec![
                r#"
//- /p1/Cargo.toml
...
enum Value {
    ...
    #[test]
    fn multiple_ratoml_in_single_source_root() {
        let server = RatomlTest::new(
            vec![
                r#"
        //- /p1/Cargo.toml
...
"#,
    ...
    #[test]
    #[ignore]
    fn non_local_crates_start_inheriting_from_client() {
        let server = RatomlTest::new(
            vec![
                r#"
//- /p1/Cargo.toml
...
fn main() {
    let a = RandomTrait;
        ...
        server.edit(1, RatomlTest::GLOBAL_TRAIT_ASSOC_ITEMS_ZERO.to_string());
        assert!(!server.query(QueryType::GlobalHover, 2));
    }

    #[test]
    fn root_ratoml_is_deletable() {
        let mut server = RatomlTest::new(
            vec![
                r#"
//- /p1/Cargo.toml
